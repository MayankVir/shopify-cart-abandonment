...
function CheckoutRow({
  ...
  const showCallButton = canInitiateCall(checkout.callStatus) && checkout.customerPhone;
  const showStopButton = canStopCall(checkout.callStatus, checkout.callScheduled);
          ...
          <div className="flex items-center justify-end gap-2">
            {showStopButton && (
              ...
              >
                ...
                {isActiveCall(checkout.callStatus) ? "Stop call" : "Cancel schedule"}
              </Button>
            )}
            {showCallButton ? (
...
export function AbandonedCheckoutsPanel() {
  ...
  const runSync = useCallback(
    (options?: { shopifyAfter?: string | null }) => {
      if (!selectedStoreDomain) return;
      ...
      startSync(async () => {
        ...
        if (result.autoCalls && result.autoCalls.processed > 0) {
          toast.info(
            `Auto-dispatched ${result.autoCalls.processed} scheduled call(s)`
          );
        }
      });
    },
  ...
  function handleBulkCancelSchedule() {
    ...
    startBulkStop(async () => {
      ...
      if (!result.success && result.stopped === 0) {
        toast.error(result.error ?? result.errors[0] ?? "Failed to cancel schedules");
        return;
      }
  ...
  return (
    <Card className="border-border/60">
      ...
      <CardContent>
        {syncWarning && (
          <div className="mb-4 rounded-lg border border-amber-500/30 bg-amber-500/10 p-3 text-xs leading-relaxed text-amber-100">
            {syncWarning}
          </div>
        )}
        {isLoadingCheckouts ? (
          ...
          <div className="space-y-3">
                ...
                <TableRow>
                  <TableHead className="w-12">
                    <Checkbox
                      ...
                      ref={(input) => {
                        if (input) {
                          input.indeterminate = someStoppableSelected;
                        }
                      }}
                      disabled={stoppableCheckouts.length === 0}
                      onChange={toggleSelectAllStoppable}
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 px-1">
              <span className="text-xs text-muted-foreground">
                Showing {checkouts.length} of {totalCount}
              </span>
